...
(defvar clw/proxy-checker
  (make-network-process
   :name "cat checker"
   :service 7897
   :host (clw/get-default-gateway)
   :nowait t))

(run-with-timer
 5.0 nil
 (lambda ()
   (when (process-live-p clw/proxy-checker)
     (let* ((gateway (clw/get-default-gateway))
            (port "7897")
            (proxy (format "%s:%s" gateway port)))
       (dolist (var '("HTTP_PROXY" "HTTPS_PROXY" "http_proxy" "https_proxy"))
         (setenv var (format "http://%s" proxy)))
        (setq url-proxy-services
        `(("http" . ,proxy)
          ("https" . ,proxy)))
     (delete-process clw/proxy-checker)
     (setq clw/proxy-checker nil)))))
;;@@ IGC-IDLE-TIMER
;; 在 Emacs 空闲时 GC
...
(define-minor-mode cwlinit-mode
